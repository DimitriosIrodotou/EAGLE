...
class DiscToTotalVsMorphologicalParameters:
    ...
    @staticmethod
    def plot(disc_fractions, kappas_corotation, disc_fractions_IT20, concentration_indices, rotationals_over_dispersions):
        """
        ...
        :param disc_fractions: where the spheroid is assumed to have zero net angualr momentum.
        ...
        plot_tools.set_axis(axis10, xlim=[2, 6.9], ylim=[0, 1], xlabel=r'$\mathrm{Concentration\;index}$', ylabel=r'$\mathrm{D/T_{30\degree}}$',
            aspect=None, which='major')
        plot_tools.set_axis(axis11, xlim=[0, 1], ylim=[0, 1], xlabel=r'$\mathrm{\kappa_{co}}$', aspect=None, which='major')
        plot_tools.set_axis(axis12, xlim=[0, 1], xlabel=r'$\mathrm{D/T_{\vec{J}_{b}=0}}$', ylim=[0, 1], aspect=None, which='major')
        ...
        for axis, axiscbar, x_attribute, threshold in zip(axes, axescbar, x_attributes, thresholds):
            # Plot attributes #
            hb = axis.hexbin(x_attribute, disc_fractions_IT20, gridsize=100, label=r'$D/T_{\vec{J}_{b} = 0}$', cmap='terrain_r')
            plot_tools.create_colorbar(axiscbar, hb, r'$\mathrm{Counts\;per\;hexbin}$', 'horizontal')

            # Plot median and 1-sigma lines #
            x_value, median, shigh, slow = plot_tools.binned_median_1sigma(x_attribute, disc_fractions_IT20, bin_type='equal_width', n_bins=20,
                log=False)
