...
class ThetaVsRadiusDistribution:
    """
    ...
    def __init__(self, simulation_path, tag):
        ...
        print('Finished ThetaVsRadiusDistribution ' + re.split('Planck1/|/PE', simulation_path)[1] + ' in %.4s s' % (time.time() - start_global_time))
        print('–––––––––––––––––––––––––––––––––––––––––––––')
    ...
    def plot(self, stellar_data_tmp, group_number, subgroup_number):
        ...
        rad = np.sqrt(
            stellar_data_tmp['Coordinates'][bulge_mask, 0] ** 2 + stellar_data_tmp['Coordinates'][bulge_mask, 1] ** 2)  # Radius of each particle.
        plot = plt.scatter(rad, angular_theta_from_densest[bulge_mask] * np.divide(180.0, np.pi),
                           c=np.log10(np.linalg.norm(prc_angular_momentum, axis=1)[bulge_mask]), cmap='nipy_spectral_r', s=3)
        cbar = plt.colorbar(plot, ax=ax)
        cbar.set_label(r'$\mathrm{|\vec{J_{prc,d}}|}$')
        
        # Save the plot #
        plt.savefig(plots_path + str(group_number) + str(subgroup_number) + '-' + 'TVRD' + '-' + date + '.png', bbox_inches='tight')
        return None
...
if __name__ == '__main__':
    ...
    plots_path = '/cosma7/data/dp004/dc-irod1/EAGLE/python/plots/TVRD/'  # Path to save plots.
    data_path = '/cosma7/data/dp004/dc-irod1/EAGLE/python/data/'  # Path to save/load data.
    if not os.path.exists(plots_path):
        os.makedirs(plots_path)
    x = ThetaVsRadiusDistribution(simulation_path, tag)
