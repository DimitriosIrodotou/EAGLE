...
class ReadAddAttirbutes:
    ...
    def __init__(self, simulation_path, tag):
        ...
        self.stellar_data, self.gaseous_data, self.subhalo_data, self.FOF_data = self.read_attributes(simulation_path, tag)
        ...
        for group_number, subgroup_number in zip(group_numbers[job_number], subgroup_numbers):  # Loop over all masked haloes and sub-haloes.
            ...
            np.save(data_path + 'gaseous_data_tmps/gaseous_data_tmp_' + str(group_number) + '_' + str(subgroup_number), stellar_data_tmp)
            np.save(data_path + 'subhalo_data_tmps/subhalo_data_tmp_' + str(group_number) + '_' + str(subgroup_number), stellar_data_tmp)
    ...
    def read_attributes(simulation_path, tag):
        ...
        FOF_data = {}
        file_type = 'FOF'
        for attribute in ['Group_M_Mean200', 'FirstSubhaloID']:
            FOF_data[attribute] = E.read_array(file_type, simulation_path, tag, '/Subhalo/' + attribute, numThreads=8)
        ...
        return stellar_data, gaseous_data, subhalo_data, FOF_data
    ...
    def mask_galaxies(self, group_number, subgroup_number):
        ...
        galaxy_mask = np.where((self.stellar_data['GroupNumber'] == group_number) & (self.stellar_data['SubGroupNumber'] == subgroup_number) & (
            self.gaseous_data['GroupNumber'] == group_number) & (self.gaseous_data['SubGroupNumber'] == subgroup_number) & (np.linalg.norm(
            np.subtract(self.stellar_data['Coordinates'], self.subhalo_data_tmp['CentreOfPotential'][halo_mask]), axis=1) <= 30.0) & (np.linalg.norm(
            np.subtract(self.gaseous_data['Coordinates'], self.subhalo_data_tmp['CentreOfPotential'][halo_mask]), axis=1) <= 30.0))  # In kpc.
        
        # Mask the temporary dictionary for each galaxy #
        stellar_data_tmp, gaseous_data_tmp = {}, {}
        ...
        for attribute in self.gaseous_data.keys():
            gaseous_data_tmp[attribute] = np.copy(self.stellar_data[attribute])[galaxy_mask]
        ...
        gaseous_data_tmp['Coordinates'] = np.subtract(gaseous_data_tmp['Coordinates'], self.subhalo_data_tmp['CentreOfPotential'][halo_mask])
...
if __name__ == '__main__':
    ...
    x = ReadAddAttirbutes(simulation_path, tag)
