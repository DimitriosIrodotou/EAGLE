...
class ResolutionTest:
    ...
    def __init__(self, simulation_path, tag):
        ...
        print('Finished ResolutionTest for ' + re.split('Planck1/|/PE', simulation_path)[1] + str(tag) + ' in %.4s s' % (time.time() - start_global_time))
        print('–––––––––––––––––––––––––––––––––––––––––––––')
    ...
    def plot(stellar_data_tmp, group_number, subgroup_number):
        ...
        plt.savefig(plots_path + str(group_number) + '_' + str(subgroup_number) + '-' + 'RT' + '-' + date + '.png', bbox_inches='tight')
        return None
...
if __name__ == '__main__':
    ...
    plots_path = '/cosma7/data/dp004/dc-irod1/EAGLE/python/plots/RT/'  # Path to save plots.
    data_path = '/cosma7/data/dp004/dc-irod1/EAGLE/python/data/'  # Path to save/load data.
    if not os.path.exists(plots_path):
        os.makedirs(plots_path)
    x = ResolutionTest(simulation_path, tag)
