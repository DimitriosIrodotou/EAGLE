...
class RotationCurve:
    def __init__(self, gn, sgn, centre):
        self.a, self.h, self.boxsize = SnapHeader.read_header()
        self.centre = centre
        # Load data.
    ...
    def read_galaxy(self, itype, gn, sgn, centre):
        ...
        gns = ReadDataset.read_dataset(itype, 'GroupNumber')
        sgns = ReadDataset.read_dataset(itype, 'SubGroupNumber')
        mask = np.logical_and(gns == gn, sgns == sgn)
        if itype == 1:
            data['mass'] = DMMass.read_dataset_dm_mass()[mask] * u.g.to(u.Msun)
        else:
            data['mass'] = ReadDataset.read_dataset(itype, 'Mass')[mask] * u.g.to(u.Msun)
        data['coords'] = ReadDataset.read_dataset(itype, 'Coordinates')[mask] * u.cm.to(u.Mpc)
